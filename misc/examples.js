    ...
    case '2':
      sepanaQuery({
        function_score: {
          ...
          script_score: {
            script: {
              source: "3 * Math.max(0, doc['totalPaid.keyword'].value.length() - 17) + 3 * Math.max(0, doc['trendingScore.keyword'].value.length() - 18)",
            },
          },
          boost_mode: "sum",
        },
...
async function sepanaQuery(query) {
  ...
  .then(json => json.hits.hits.forEach(el => console.log(`${el._score} v${el._source.pv}, project ${el._source.projectId}: ${el._source.name}. ${el._source.totalPaid / 1e18} ETH Paid. Trending: ${el._source.trendingScore / 1e18}.`)))
}
